...
def train_model(model, loss_fn, optimizer, train_generator, dev_generator):
    ...
    while True:
        ...
        for Xdata, ydata in train_generator:
            # Predict (forward)
            model.zero_grad()
            pred = model(Xdata)
            
            # Back-propagate
            loss = loss_fn(pred, ydata)
            loss.backward()
        ...
        curLoss = 0
        for Xdata, ydata in dev_generator:
            curLoss += loss_fn(model(Xdata), ydata).item()

        # Display info for current epoch
        print("Epoch", nEpoch, "- loss:", curLoss)
        nEpoch += 1

        # Stop early if loss is increasing
        if (prevLoss < curLoss) or (nEpoch > 50):
        # if (prevLoss < curLoss):
            break
        else:
            prevLoss = curLoss
...
def main(args):
    ...
    if FRESH_START:
        ...
        with open(TEMP_FILE, "wb+") as f:
            ...
            # torch.save((train_generator, dev_generator, test_generator, embeddings, train_data), TEMP_FILE) # torch.save ?
    else:
        try:
            with open(TEMP_FILE, "rb") as f:
                print("Loading DataLoaders and embeddings from file....")
                train_generator, dev_generator, test_generator, embeddings, train_data = pickle.load(f)
                # train_generator, dev_generator, test_generator, embeddings, train_data = torch.load(TEMP_FILE)
    ...
    myRNN = models.RecurrentNetwork(embeddings, NUM_CLASSES)
    optimizerAdam = optim.Adam(myRNN.parameters())
    myRNN = train_model(myRNN, loss_fn, optimizerAdam, train_generator, dev_generator)
    test_model(myRNN, loss_fn, test_generator)
    torch.save(myRNN.state_dict(), 'model.pkl')
